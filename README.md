...
Bonus Features
Search Integration
The assignment lists external search integration as an optional bonus.

A search provider such as:

SerpAPI
Tavily
Custom browser search
could be added to discover external LinkedIn profiles when they are not present on the company website.

The current core implementation intentionally limits LinkedIn extraction to URLs explicitly discoverable from the supplied website content.

This avoids fabricating or incorrectly attributing external profiles.

Agentic Navigation
The crawler can be extended with frameworks such as:

Browser-Use
LangGraph
or a custom multi-step tool-calling loop.

An agentic version could dynamically decide which pages to visit based on information discovered during crawling.

Cost Tracking
Approximate token and API cost tracking is already included.

Security & Configuration Notes
API Keys
API keys must only be stored in .env.
...
Public Web Content
The application is intended to process publicly accessible company websites.

It should not be used to bypass authentication, access private information, or circumvent website security mechanisms.
...
Do not manually fabricate the sample results.

A real generated output demonstrates that the crawler, preprocessing pipeline, Gemini extraction, structured validation, and error handling work together end-to-end.

Submission Checklist
Before submitting the assignment, verify:

 GitHub repository is accessible
 Repository contains clean, modular Python code
 .env is not committed
 .env.example is included
 requirements.txt is included
 Playwright Chromium is installed
 Gemini API key works
 python -m app.main postman.com supabase.com vapi.ai runs successfully
 output/output.json contains results for all three domains
 Company overview is present
 Target audience / ICP is present
 Contact points are extracted
 Leadership/team information is included when discoverable
 Confidence scores are included
 Scraped page URLs are recorded
 Errors are recorded rather than crashing the entire run
 Automated tests pass
 Loom recording demonstrates the code, execution, and output
 The 40% manual operations question is answered explicitly in the submission
 LinkedIn profile is included in the submission email/form
 Sample output was generated from an actual run
Loom Walkthrough
The assignment requests a 2–3 minute walkthrough.
...
Playwright → preprocessing → Gemini → Pydantic → JSON
...
Page limits and timeouts
0:50–1:15 — Content Preprocessing
Open:

app/extractor.py

Explain that raw HTML is not sent directly to the LLM.

Show that scripts, styles, SVGs, navigation, footer content, and other unnecessary DOM elements are removed before extraction.

1:15–1:35 — Structured Extraction
Open:
...
A useful explanation during the walkthrough is:

The key design decision was to avoid sending raw HTML to the LLM. Playwright handles JavaScript rendering, then the pipeline removes non-content DOM elements and only passes cleaned, bounded text into a Pydantic-validated structured extraction pipeline.

Operations Requirement
The assignment includes the following mandatory screening question:
...
This question should be answered explicitly in the final submission email/form.
...
Submission Email
The assignment specifies the following submission email:

support@softwarebrio.com

The required subject format is:
...
LinkedIn profile
Confirmation regarding the 40% manual operations requirement
Loom recording
Any other requested assignment materials
Future Improvements
Potential extensions include:

Search-engine integration for external LinkedIn discovery
Dynamic agentic navigation
Robots.txt-aware crawling
Persistent crawl cache
Concurrent domain processing
More sophisticated content relevance scoring
Email verification
Structured source citations for each extracted field
Additional CRM-ready output formats such as CSV
Improved retry/backoff strategies for rate limiting
Provider-independent LLM interface
More comprehensive automated tests
Better extraction of structured company metadata
Per-field evidence and source URLs
Current Scope
The current implementation prioritizes the core assignment requirements:

Automated browser-based website crawling
JavaScript-rendered content retrieval
Relevant page discovery
Clean content preprocessing
